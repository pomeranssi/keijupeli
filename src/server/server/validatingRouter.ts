...
import { MaybePromise, ObjectIdString, SessionInfo } from 'shared/types';
import { isDefined, recordFromPairs } from 'shared/util';
  ...
  const txFuns = recordFromPairs(
    RouteMethods.map<[`${RouteMethod}Tx`, ValidatedTxRequest]>(m => [
      ...
      (path, spec, handler) =>
        router[m](
          path,
          Requests.validatedTxRequest(addParamType(path, spec), handler)
        ),
    ])
  );
  ...
  const funs = recordFromPairs(
    RouteMethods.map<[RouteMethod, ValidatedRequest]>(m => [
      ...
      (path, spec, handler) =>
        router[m](
          path,
          Requests.validatedRequest(addParamType(path, spec), handler)
        ),
    ])
  );
